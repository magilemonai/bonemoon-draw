...
// One card, whole, at a size that can be read, with a turn to its other face.
function CodexCard({ def, onInspect }: { def: CardDef; onInspect: () => void }) {
  const [face, setFace] = useState<Face>('upright')
  const other: Face = face === 'upright' ? 'reversed' : 'upright'
  return (
    <div className={`codex-card ${face === 'reversed' ? 'is-reversed' : ''}`}>
      <Card def={def} face={face} size="full" onClick={onInspect} />
      <div className="codex-card-foot">
        <span className="codex-card-face">{face === 'upright' ? 'Upright' : 'Reversed'}</span>
        <button type="button" className="btn codex-turn" onClick={() => setFace(other)} aria-label={`Turn ${def.name} over to its ${other} face`}>
          Turn over
        </button>
      </div>
    </div>
...
export function Codex() {
  ...
  const [view, setView] = useState<'paintings' | 'cards' | 'library'>('cards')
  ...
  return (
    <div className="codex">
      <div className="codex-head">
        ...
        <h2>The Codex</h2>
        <span className="codex-sub">Seventy-eight cards, whole. Turn any of them over. Tap one for the painting in full, or build a deck from them.</span>
      ...
      {view === 'library' ? (
        ...
        <div className="codex-grid">
          {cards.map((c) => (
            <CodexCard key={c.id} def={c} onInspect={() => inspect(c.id, 'upright')} />
          ))}
        </div>
      ) : (
