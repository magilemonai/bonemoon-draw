...
export function FlipIn({ children, delay = 0, duration = 0.55, className = '' }: { children: ReactNode; delay?: number; duration?: number; className?: string }) {
  ...
  const speed = useStore((s) => s.speed) || 1 // Quick shortens the turn as it shortens everything else
  return (
    <motion.div className={`card-3d ${className}`} initial={{ rotateY: 180 }} animate={{ rotateY: 0 }} transition={reduce ? { duration: 0 } : { duration: duration / speed, delay: delay / speed, ease: [0.2, 0.7, 0.2, 1] }}>
