...
The engine-run preview was checked again and the Tidecaller case confirmed fixed. Three more constructed cases showed the formatter still guessing where the engine already knew: Kaipo, who moves after attacking, was reported dead because his lane was empty afterwards; a blocked retaliation was shown as "takes nothing" even though a Last Rite then dealt 2; and a Rekindle was found by the outcome and never spoken, because the line only looked for it under a death. All three are fixed. The outcome now finds each Figure by identity after the attack (on the table, moved, back in hand, or gone), reads a Rekindle from the events, and reports damage totals with "(a hit blocked)" beside them when a hit was stopped, whether by Aegis or by damage reduction. The lines read "Yours moves to Past at 1", "Deals 6, takes 2 (a hit blocked)", "It rekindles at 1", and "It rekindles, then falls" when a revival does not last. The formatter now lives beside the outcome in `src/engine/preview.ts`, so it is tested there, with the three cases as expectations for the corrected display (74 tests in all).
